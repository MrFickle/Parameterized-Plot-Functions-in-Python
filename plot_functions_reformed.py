...
def plot_parameterized_barplot_two_y_axis(data_dictionary, xlabel, ylabel_left, ylabel_right, figure_title, filename, data_path_out,
                                          color_dict_bars, color_dict_legends, legend_dictionary, bins, bar_width, xticks, xticks_labels,
                                          legends, align='edge', alpha=1.0, edgecolor=None, rotate_xticks=False, sem_data_dictionary=None,
                                          remove_first_y_left=False, remove_first_y_right=False, xlims=None, ylims_left=None, ylims_right=None,
                                          figure_size=(25, 25), legend_loc='upper right', ncol=2, font_size=90, label_size=75,
                                          capsize=20, elinewidth=5, capthick=3, yticks_left=None, yticks_right=None, yticks_labels_left=None,
                                          yticks_labels_right=None, plot_minor_ticks=False, spine_width=7, yticks_font_size=None, xticks_font_size=None,
                                          set_ylims_one_extra_tick=False, pad=0.25, save_svg=True, place_text=None, place_text_loc=None,
                                          text_font_size=None, make_text_bold=False, place_text_color=None, use_sns=True, disable_xtick_edges=False,
                                          bbox_to_anchor=None, xlabel_pad=20, legend_axis_assignment=None, return_fig_instead_of_save=False):
    """
    Creates and optionally saves a parameterized bar plot with dual Y-axes based on provided data,
    customization options, and plot settings.
    ...
    - color_dict_bars (dict): Colors for each bar {'Bar1': 'color1', 'Bar2': 'color2', ...}.
    - color_dict_legends (dict): Colors for each legend {'Legend1': 'color1', 'Legend2': 'color2', ...}.
    - legend_dictionary (dict): Legend names for each distribution {'Dist1': 'Legend1', 'Dist2': 'Legend2', ...}.
    - bins (dict): X-axis positions of each bar {'Bar1': xvalue1, 'Bar2': xvalue2, ...}.
    - bar_width (dict): Width of each bar {'Bar1': width1, 'Bar2': width2, ...}.
    - xticks (numpy.array/list): Values to show as ticks on the X-axis.
    - xticks_labels (numpy.array/list): Labels to overwrite the ticks on the X-axis.
    ...
    - rotate_xticks (bool, optional): Whether to rotate the X-axis ticks (True for vertical, False for horizontal).
    - sem_data_dictionary (dict, optional): Standard error of the mean for each bar {'Bar1': sem1, 'Bar2': sem2, ...}.
    - remove_first_y_left (bool, optional): Whether to remove the first tick on the left Y-axis to avoid overlap.
    - remove_first_y_right (bool, optional): Whether to remove the first tick on the right Y-axis to avoid overlap.
    - xlims (list, optional): Minimum and maximum values for the X-axis [min_value, max_value].
    - ylims_left (list, optional): Minimum and maximum values for the left Y-axis [min_value, max_value].
    - ylims_right (list, optional): Minimum and maximum values for the right Y-axis [min_value, max_value].
    - figure_size (tuple, optional): Width and height of the figure (width, height).
    - legend_loc (str, optional): Location of the legend.
    - ncol (int, optional): Number of columns in the legend.
    - font_size (int, optional): Font size for title, labels, and legend.
    ...
    - elinewidth (int, optional): Line width of the error bars.
    - capthick (int, optional): Thickness of the error bar caps.
    - yticks_left (numpy.array/list, optional): Values to show as ticks on the left Y-axis.
    - yticks_right (numpy.array/list, optional): Values to show as ticks on the right Y-axis.
    - yticks_labels_left (numpy.array/list, optional): Labels to overwrite the ticks on the left Y-axis.
    - yticks_labels_right (numpy.array/list, optional): Labels to overwrite the ticks on the right Y-axis.
    - plot_minor_ticks (bool, optional): Whether to show minor ticks on axes.
    - spine_width (int, optional): Width of the axes spines.
    - yticks_font_size (int, optional): Font size for Y-axis ticks, if different from label_size.
    - xticks_font_size (int, optional): Font size for X-axis ticks, if different from label_size.
    - set_ylims_one_extra_tick (bool, optional): Expand Y-axis limits to include one extra tick beyond the data range.
    - pad (float, optional): Padding used in layout adjustment.
    - save_svg (bool, optional): Whether to save the figure in SVG format in addition to PNG.
    - place_text (list, optional): Texts to annotate on the plot.
    - place_text_loc (list, optional): [x, y] positions for each text annotation.
    - text_font_size (list, optional): Font sizes for text annotations.
    - make_text_bold (bool, optional): Whether annotated texts should be bold.
    - place_text_color (list, optional): Colors for text annotations.
    - use_sns (bool, optional): Whether to use seaborn for setting some plot parameters.
    - disable_xtick_edges (bool, optional): Whether to remove tick lines from the X-axis.
    - bbox_to_anchor (tuple, optional): Location for the legend's bounding box anchor.
    - xlabel_pad (int, optional): Padding between the X-label and the axis.
    - legend_axis_assignment (dict, optional): Maps each legend to either the 'left' or 'right' Y-axis {'Legend1': 'left', 'Legend2': 'right'}.
    - return_fig_instead_of_save (bool, optional): If True, returns the figure object instead of saving.
    ...
    fs_dict = {
        'xlabel': font_size,
        'ylabel': font_size,
        'xticks': xticks_font_size or label_size,
        'yticks': yticks_font_size or label_size,
        'legend': font_size,
        'title': font_size
    }
    ...
    # Plotting bars
    for bar, height in data_dictionary.items():
        ax = ax1 if bar in legend_axis_assignment and legend_axis_assignment[bar] == 'left' else ax2
        yerr = sem_data_dictionary.get(bar, None) if sem_data_dictionary else None
        ax.bar(bins[bar], height, width=bar_width[bar], color=color_dict_bars[bar], alpha=alpha, yerr=yerr,
               capsize=capsize, edgecolor=edgecolor.get(bar, 'black') if edgecolor else 'black',
               error_kw={'elinewidth': elinewidth, 'capthick': capthick}, align=align)

    customize_axes(ax1, xlabel, ylabel_left, xticks, yticks_left, xticks_labels, yticks_labels_left, xlims, ylims_left, fs_dict,
                   tick_width, capsize, spine_width, True, True, False)

    customize_axes(ax2, '', ylabel_right, [], yticks_right, [], yticks_labels_right, [], ylims_right, fs_dict,
                   tick_width, capsize, spine_width, False, True, False)

    if rotate_xticks:
        plt.setp(ax1.get_xticklabels(), rotation=90)

    # Handling legend
    if legends and legend_dictionary:
        handles = []
        for legend in legends:
            handles.append(plt.Line2D([0], [0], color=color_dict_legends[legend], lw=4, label=legend_dictionary[legend]))
        ax1.legend(handles=handles, loc=legend_loc, ncol=ncol, frameon=False, bbox_to_anchor=bbox_to_anchor)

    # Text annotations
    if place_text and place_text_loc:
        annotate_text(ax1, place_text, place_text_loc, text_font_size if text_font_size else [font_size for _ in place_text],
                      make_text_bold if make_text_bold else [False for _ in place_text],
                      place_text_color if place_text_color else ['black' for _ in place_text])
...
def parameterized_heatmap(data, title, xlabel, ylabel, data_path_out, filename,
                          font_size=45, label_size=40, figure_size=(20, 20), plot_colorbar=True,
                          vmin=None, vmax=None, rotate_ticks=False, cmap='viridis',
                          return_fig_instead_of_save=False):
    """
    Generates and optionally saves a heatmap based on a provided DataFrame.

    Parameters:
    ----------
    - data (pd.DataFrame): The data to be plotted as a heatmap. Indexes and columns will be used for Y and X axes.
    - title (str): Title of the figure.
    - xlabel (str): Label for the X-axis.
    - ylabel (str): Label for the Y-axis.
    - data_path_out (str): Absolute path of the folder to store the figure. Folder must exist prior to calling.
    - filename (str): Name of the file (without file extension) to save the plot.
    - font_size (int, optional): Font size for the title, axis labels, and colorbar label. Default is 45.
    - label_size (int, optional): Font size for the axis tick labels. Default is 40.
    - figure_size (tuple, optional): Size of the figure in the format (width, height). Default is (20, 20).
    - plot_colorbar (bool, optional): Flag indicating whether to include a colorbar. Default is True.
    - vmin (int/float, optional): Minimum value for the colormap scale. Default is None, which sets it to data min value.
    - vmax (int/float, optional): Maximum value for the colormap scale. Default is None, which sets it to data max value.
    - rotate_ticks (bool, optional): Flag indicating whether to rotate axis tick labels. Default is False.
    - cmap (str, optional): Colormap for the heatmap. Default is 'viridis'.
    - return_fig_instead_of_save (bool, optional): If True, returns the figure object instead of saving. Default is False.

    Returns:
    ----------
    - matplotlib.figure.Figure or None: The figure object if `return_fig_instead_of_save` is True. Otherwise, None.
    """
...
def draw_multiple_figures_as_subplots_at_single_figure(figures_list, figure_rows, data_path_out, filename, grid_hspace=0, return_fig_instead_of_save=False,
    ...
    """
    Merges multiple figures into a single figure as subplots arranged by specified rows, allowing for customized subplot arrangements.

    Parameters:
    ----------
    - figures_list (list): List of matplotlib.figure.Figure objects to be merged into subplots.
    - figure_rows (list): List of row indices indicating the row position for each figure in 'figures_list'.
    - data_path_out (str): Absolute path of the directory to save the merged figure.
    - filename (str): Name of the file (without file extension) for saving the merged figure.
    - grid_hspace (float, optional): The amount of height reserved for white space between subplots, expressed as a fraction of the average axis height. Default is 0.5.
    - return_fig_instead_of_save (bool, optional): If True, returns the merged figure object instead of saving. Default is False.
    - place_text (list, optional): List of strings containing texts to annotate on the merged figure.
    - place_text_loc (list, optional): List of [x, y] positions for each text annotation, relative to the figure coordinate system.
    - text_font_size (list, optional): List of font sizes for each text annotation.
    - make_text_bold (bool, optional): If True, renders all text annotations in bold. Default is False.
    - place_text_color (list, optional): List of colors for each text annotation.
    - rotate_place_text (list, optional): List of booleans indicating whether to rotate each text annotation. Default is False for all.
    - pad (float, optional): Padding around the merged figure, affecting tight_layout adjustment. Default is 0.5.

    Returns:
    ----------
    - matplotlib.figure.Figure or None: The merged figure object if `return_fig_instead_of_save` is True; otherwise, None.

    IMPORTANT NOTES:
    ----------
    1) When merging individual figures and placing text, ensure consistent padding across individual figures and adjust text font size based
    on the formula provided.

    2) When merging individual figures with an already merged figure from this function, ensure consistent padding and adjust new
    individual figure font sizes accordingly.
    """
    # Total figures
